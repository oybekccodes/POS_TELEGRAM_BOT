...
PIN_CODE = os.getenv("PIN_CODE", "010203")
...
async def today(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT SUM(price) FROM sales WHERE date=%s", (today_str,))
...
async def add_barcode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM products WHERE barcode=%s", (code,))
...
async def add_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO products (barcode, name, price, date_added)
            VALUES (%s, %s, %s, %s)
            ON CONFLICT (barcode) DO UPDATE SET name=EXCLUDED.name, price=EXCLUDED.price, date_added=EXCLUDED.date_added
...
async def search_by_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute("SELECT id, barcode, name, price, date_added FROM products WHERE id=%s", (pid,))
        row = cursor.fetchone()
        cursor.close()
        conn.close()
        if row:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("scan_mode"):
        ...
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute("SELECT id, name, price FROM products WHERE barcode=%s", (code,))
        row = cursor.fetchone()
        if row:
            pid, name, price = row
            today_str = datetime.now().strftime("%Y-%m-%d")
            cursor.execute("INSERT INTO sales (product_id, price, date) VALUES (%s, %s, %s)", (pid, price, today_str))
            conn.commit()
    ...
    try:
        ...
        if code:
            ...
            if row:
                ...
                today_str = datetime.now().strftime("%Y-%m-%d")
                cursor.execute(
                    "INSERT INTO sales (product_id, price, date) VALUES (%s, %s, %s)",
                    (pid, price, today_str)
                )
                conn.commit()
                await update.message.reply_text(f"{name} sotildi ✅\nNarx: {price}")
